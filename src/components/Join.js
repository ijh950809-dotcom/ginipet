...
function Join(props) {
  ...
  const handleChange = (e) => {
    setForm({
      ...form,//기존폼데이터에 이어서 저장
      [e.target.name]: e.target.value//사용자가 입력한 데이터를 각각저장
    });
    //데이터값 없는 경우 
    setMessage('');//에러초기화
  }
  ...
  //4. 아이디 중복체크 기능
  const checkUsername = () => {
    axios.post('https://port-0-backend-express-server-mkvwe8rkb7d9ea30.sel3.cloudtype.app/check-username', {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://port-0-backend-express-server-mkvwe8rkb7d9ea30.sel3.cloudtype.app/ginipet_register', form)
      .then(() => {//전송성공시
        alert('회원가입이 완료되었습니다. 로그인 페이지로 이동합니다.');
        navigate('/login');//로그인 경로주소
      })
      .catch(err => {//실패시
        console.log(err);//콘솔모드에 에러 출력하고
        setMessage('회원가입실패:아이디가 이미 존재하거나 서버오류입니다.');
        setError(true);
      });
  };
