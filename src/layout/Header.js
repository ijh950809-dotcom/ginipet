...
function Header(props) {
  ...
  // 로그아웃 버튼 클릭시 실행되는 함수
  const handleLogout = () => {
    localStorage.removeItem('token');//기존로그인 데이터 삭제
    localStorage.removeItem('username');//사용자 아이디 데이터 삭제
    navigate('/login');//로그인 페이지로 이동하기
    window.location.reload();//상태갱신을 위해 새로고침(선택)
  };
  ...
  return (
    <>
      <header className='header'>
        <h1>
          <Link to='/' title='메인페이지로 바로가기'><img src={`${process.env.PUBLIC_URL}/images/logo_clr.png`} alt="상단로고" /></Link>
        </h1>
        ...
        >
          ...
          <ul className='gnb'>
            <li className='member_info'>
              {/* '님 환영합니다' 조건부렌더링 공식에 의해 아이디가 있을 경우 '님 환영합니다.' 나오도록함 */}
              <b>{username}</b>{username && '님 환영합니다.'}
            </li>
